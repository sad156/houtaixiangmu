<template>
  <div class="app-container">
    ...
    >
      ...
      <!-- 套餐名称 -->
      <el-table-column :label="$t('table.package.name')" align="center" width="250">
        <template slot-scope="scope">
      ...
      <!-- 套餐金额 -->
      <el-table-column :label="$t('table.package.flowmoney')" align="center" width="120">
        <template slot-scope="scope">
      ...
      <!-- 是否首冲 -->
      <el-table-column :label="$t('table.package.isFirst')" align="center" width="100">
        <template slot-scope="scope">
          <span>{{ scope.row.isFirst }}</span>
        </template>
      </el-table-column>

      <!-- 是否公开 -->
      <el-table-column :label="$t('table.package.isPublic')" align="center" width="100">
        <template slot-scope="scope">
      ...
      <!-- 时长 -->
      <el-table-column :label="$t('table.package.times')" align="center" width="80">
        <template slot-scope="scope">
      ...
      <!-- 型号 -->
      <el-table-column :label="$t('table.package.ename')" align="center" width="200">
        <template slot-scope="scope">
      ...
      <!-- 计费方式 -->
      <el-table-column :label="$t('table.package.billName')" align="center" width="100">
        <template slot-scope="scope">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="right" label-width="100px" style="width: 100%;">
        <div style="display:flex;width:100%">
          <div style="width:50%">
            <el-form-item :label="$t('table.package.name')">
              <el-input v-model="temp.name" />
            </el-form-item>
            <el-form-item :label="$t('table.package.times ')" prop="times ">
              <el-input v-model="temp.times " />
            </el-form-item>
            <el-form-item :label="$t('table.package.isFirst')" prop="isFirst">
              <el-radio-group v-model="temp.isFirst">
                <el-radio v-for="item in isFirst" :label="item.key">{{ item.display_name }}</el-radio>
              </el-radio-group>
            </el-form-item>
            <el-form-item :label="$t('table.package.isPublic')" prop="isPublic">
          ...
          <div style="width:50%;margin-left:10px">
            ...
            <el-form-item :label="$t('table.package.flowmoney')" prop="flowmoney">
              <el-input v-model="temp.flowmoney" />
            </el-form-item>
            <el-form-item :label="$t('table.package.ename')">
              <wlTreeSelect v-model="temp.equipmentId" :data="treeData" style="width:177.75px;" @change="hindleChanged" />
            </el-form-item>
            <el-form-item :label="$t('table.package.billName')" prop="isFirst">
              <el-radio-group v-model="temp.bill" style="font-size:28px;">
                <el-radio v-for="item in bill" :label="item.key">{{ item.display_name }}</el-radio>
              </el-radio-group>
            </el-form-item>
...
import { addcombo, queryname, editcombo, del, list, queryById } from '@/api/package'
